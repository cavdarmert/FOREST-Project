...
<body>
	...
	<c:forEach var="item" items="${allItems}">
		<div class="single-item-container">			
			<c:if test = "${hero.gold < item.price}">
			<h3 style="color:red" >Not Enough Gold to Buy <c:out value="${item.name}"/></h3>
			</c:if>
			<c:if test = "${hero.gold >= item.price}">
			<h4><a href="/trader/buy/${item.id}">Buy <c:out value="${item.name}"></c:out></a></h4>
			</c:if>
			<p>HEALING: <c:out value="${item.healingPercent}"></c:out>%</p>
			<p>CRIT CHANCE: +<c:out value="${item.criticalChanceAddition}"></c:out></p>
			<p>DEFENSE: +<c:out value="${item.defensePowerAddition}"></c:out></p>
			<p>ATTACK: +<c:out value="${item.attackPowerAddition}"></c:out></p>
			<p>PRICE: <c:out value="${item.price}"></c:out></p>
			<p>CATEGORY: <c:out value="${item.category}"></c:out></p>
