	...
	<c:forEach var="game" items="${allGames}">
		<div class="single-item-container">			
			...
			<c:if test = "${(hero.diamond >= game.ticketPrice) && (game.priceType =='DIAMOND')}">
				...
				<input class="input" class="button" type="submit" value="Play the Game"/>
				</form:form>
			</c:if>
			<c:if test = "${(hero.gold >= game.ticketPrice) && (game.priceType =='GOLD')}">
				<form:form action="/games/play/${game.id}/" method="post" modelAttribute="hero">
			...
			<p>REWARD: <c:out value="${game.rewardAmount}"/> <c:out value="${game.rewardType}"/></p>				
			<p>ENTRY: <c:out value="${game.ticketPrice}"/> <c:out value="${game.priceType}"/></p>
			<p><c:out value="${game.name}"/></p>
				
			<div>
				<img width="100" height="100" src="<c:url value="/images/game_big.png" />" />
			</div>
